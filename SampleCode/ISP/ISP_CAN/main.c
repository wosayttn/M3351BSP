 ...
 * @file     main.c
 * @version  V1.00
 * @brief    Demonstrate how to update chip flash data through CAN interface.
 *           Nuvoton NuMicro ISP Programming Tool is also required in this
 *           sample code to connect with chip CAN and assign update file
 *           of Flash.
 *
    ...
    while (1)
    {
        if (s_u8CANPackageFlag)
        {
            ...
            else    // Update user config or APROM
            {
                ...
                if ((u32Addr & 0x00300000UL) == 0x00300000UL)   /* User Config Address for CAN */
                {
                    u32Addr = (u32Addr | FMC_CONFIG_BASE);
                ...
                else
                {
                    ...
                    {
                        FMC_Write8Bytes((u32Addr & 0xFFFFFFF8), u32LSWData, psISPCanMsg->Data);

                        /* read back LSW data */
                        psISPCanMsg->Data = FMC_Read(u32Addr & 0xFFFFFFF8);
                    }
                }
            }
