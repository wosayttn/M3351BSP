...
void FMC_ISP(uint32_t u32Cmd, uint32_t u32Addr, uint32_t u32Data0, uint32_t u32Data1)
{
    FMC->ISPCMD  = u32Cmd;
    FMC->ISPADDR = u32Addr;

    if (u32Cmd == FMC_ISPCMD_PROGRAM_64)
    {
        FMC->MPDAT0  = u32Data0;
        FMC->MPDAT1  = u32Data1;
    }
...
uint32_t FMC_Init(void)
{
    ...
    FMC_ISP(FMC_ISPCMD_READ, FMC_ARLOCK_BASE, 0, 0);
...
void DataFlashProgramPage(uint32_t u32StartAddr, uint32_t *u32Buf)
{
    ...
    for (i = 0; i < STORAGE_BUFFER_SIZE / 4; i += 2)
    {
        FMC_ISP(FMC_ISPCMD_PROGRAM_64, (u32StartAddr + (i * 4)), u32Buf[i], u32Buf[i + 1]);
    }
}
    ...
    if ((u32Addr >= DATA_SEC_ADDR) && (u32Addr < g_u32StorageSize))
    {
        ...
        else
        {
            if ((u32Addr & (FMC_FLASH_PAGE_SIZE - 1)) == 0)
                FMC_ISP(FMC_ISPCMD_PAGE_ERASE, (FMC_APROM_BASE + u32Addr), 0, 0);
        }
