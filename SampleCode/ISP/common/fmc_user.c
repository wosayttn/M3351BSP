...
int FMC_Proc(uint32_t u32Cmd, uint32_t u32StartAddr, uint32_t u32EndAddr, uint32_t *pu32Data)
{
    ...
    for (u32Addr = u32StartAddr; u32Addr < u32EndAddr;)
    {
        ...
        else if (u32Cmd == FMC_ISPCMD_PROGRAM_64)
        {
            FMC->MPDAT0 = *pu32Data;
            pu32Data++;
            FMC->MPDAT1 = *pu32Data;
        ...
        u32RegVal = FMC->ISPCTL;

        if (u32RegVal & FMC_ISPCTL_ISPFF_Msk)
        {
            FMC->ISPCTL = u32RegVal;
            return -1;
        }
...
{
    FMC_Proc(FMC_ISPCMD_PROGRAM_64, u32StartAddr, u32EndAddr, pu32Data);
    return;
}
...
{
    uint32_t u32ConfigBytes = (ISP_CONFIG_CNT * 4);

    FMC_ENABLE_CFG_UPDATE();

    FMC_Proc(FMC_ISPCMD_CFG_ERASE, FMC_USER_CONFIG_0, (FMC_USER_CONFIG_0 + u32ConfigBytes), 0);
    FMC_Proc(FMC_ISPCMD_PROGRAM,   FMC_USER_CONFIG_0, (FMC_USER_CONFIG_0 + u32ConfigBytes), pu32Data);

    if (pu32Resp)
    {
        /* Read back to response */
        FMC_Proc(FMC_ISPCMD_READ, FMC_USER_CONFIG_0, (FMC_USER_CONFIG_0 + u32ConfigBytes), pu32Resp);
    }
