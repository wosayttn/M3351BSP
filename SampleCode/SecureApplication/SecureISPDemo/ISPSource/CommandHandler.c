...
/*---------------------------------------------------------------------------------------------------------*/
    ...
    #define CURVE_P_SIZE        MBEDTLS_ECP_DP_SECP256R1
#endif
...
static int32_t _WriteFlash(uint32_t u32Addr, uint32_t u32Size, uint32_t *pu32Data, uint32_t u32CmdMask)
{
    ...
    for (i = 0; i < (u32Size / 4); i += 2)
    {
        /* write */
        if (_ISPWrite(u32Addr, pu32Data[i], pu32Data[i + 1]) != 0)
        {
            ret = ERR_ISP_WRITE;
            break;
        }
        ...
        /* verify */
        if ((_ISPRead(u32Addr) != pu32Data[i]) || (_ISPRead(u32Addr + 4) != pu32Data[i + 1]))
        {
            ret = ERR_ISP_WRITE;
            break;
        }
