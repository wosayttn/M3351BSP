...
static int32_t ECDH_3_GetPub1(void)
{
    ...
    /* [Device Authentication] Verify received Public Key against expected key.
     * Disabled to allow development flexibility and random ephemeral keys.
     * Enable this block to enforce trusted device whitelisting. */
...
static int32_t Process_ECDH(void)
{
    ...
    if ((i32RetCode = ECDH_0_SendPub0()) != 0)
    {
        DBG("\n[FAIL] ==> ECDH_0_SendPub0 (Return %d).\n", i32RetCode);
        return -1;
    }

    if ((i32RetCode = ECDH_1_SendPub1()) != 0)
    {
        DBG("\n[FAIL] ==> ECDH_1_SendPub1 (Return %d).\n", i32RetCode);
        return -1;
    }

    /* The following commands are encrypted by 1st ECDH key */
    if ((i32RetCode = ECDH_2_GetPub0()) != 0)
    {
        DBG("\n[FAIL] ==> ECDH_2_GetPub0 (Return %d).\n", i32RetCode);
        return -1;
    }

    if ((i32RetCode = ECDH_3_GetPub1()) != 0)
    {
        DBG("\n[FAIL] ==> ECDH_3_GetPub1 (Return %d).\n", i32RetCode);
        return -1;
    }

    if ((i32RetCode = ECDH_4_SendRandPub0()) != 0)
    {
        DBG("\n[FAIL] ==> ECDH_4_SendRandPub0 (Return %d).\n", i32RetCode);
        return -1;
    }

    if ((i32RetCode = ECDH_5_SendRandPub1()) != 0)
    {
        DBG("\n[FAIL] ==> ECDH_5_SendRandPub1 (Return %d).\n", i32RetCode);
        return -1;
    }

    if ((i32RetCode = ECDH_6_GetRandPub0()) != 0)
    {
        DBG("\n[FAIL] ==> ECDH_6_GetRandPub0 (Return %d).\n", i32RetCode);
        return -1;
    }

    if ((i32RetCode = ECDH_7_GetRandPub1()) != 0)
    {
        DBG("\n[FAIL] ==> ECDH_7_GetRandPub1 (Return %d).\n", i32RetCode);
        return -1;
    }
...
int32_t Process_USBHCommand(HID_DEV_T *hdev)
{
    ...
    uint32_t u32Item, u32CmdID,
             u32FlashPageCnt   = 2,
             u32FlashStartAddr = (uint32_t)(FMC_APROM_BASE + 0x40000),
             u32FlashEndAddr   = (uint32_t)(FMC_APROM_BASE + 0x40000 + (FMC_FLASH_PAGE_SIZE * u32FlashPageCnt));
