...
int32_t ECC_VerifySignature(CRYPTO_T *psCRYPTO, E_ECC_CURVE eECC_Curve, char *message, char *public_k1, char *public_k2, char *R, char *S)
{
    ...
    mbedtls_mpi r, s;
    mbedtls_mpi hash;
    unsigned char hash_buf[MBEDTLS_ECP_MAX_BYTES];
    size_t hash_len;
    ...
    ret = mbedtls_ecp_group_load(&grp, eECC_Curve);

    if (ret != 0)
        goto cleanup;

    /* Load public key point Q */
    ret = mbedtls_ecp_point_read_string(&Q, 16, public_k1, public_k2);

    if (ret != 0)
        goto cleanup;

    /* Load signature components */
    ret = mbedtls_mpi_read_string(&r, 16, R);

    if (ret != 0)
        goto cleanup;

    ret = mbedtls_mpi_read_string(&s, 16, S);

    if (ret != 0)
        goto cleanup;

    /* Load message hash and convert to binary */
    ret = mbedtls_mpi_read_string(&hash, 16, message);

    if (ret != 0)
        goto cleanup;

    hash_len = (grp.nbits + 7) / 8;
    ret = mbedtls_mpi_write_binary(&hash, hash_buf, hash_len);
...
int32_t  ECC_GenerateSecretZ(CRYPTO_T *psCRYPTO, E_ECC_CURVE eECC_Curve, char *private_k, char public_k1[], char public_k2[], char secret_z[])
{
    ...
    mbedtls_ecp_point_init(&z_point);
    ...
    // Compute shared secret Z = d * Qp (scalar multiplication)
    // This is the ECDH shared secret computation using public API
    if ((i32RetCode = mbedtls_ecp_mul(&grp, &z_point, &d, &Qp,
                                      mbedtls_ctr_drbg_random, &ctr_drbg)) != 0)
    {
        goto cleanup;
    }

    // Export the point to uncompressed format (04 + x + y) using public API
    keylen = (grp.nbits + 7) / 8;
    {
        unsigned char point_buf[MBEDTLS_ECP_MAX_PT_LEN];
        size_t point_len;

        if ((i32RetCode = mbedtls_ecp_point_write_binary(&grp, &z_point,
        ...
        // Extract x-coordinate (skip the first byte 0x04 format indicator)
        // The x-coordinate starts at point_buf[1] and has length keylen
        if (point_len < 1 + keylen)
        {
            i32RetCode = -1;
            goto cleanup;
        }

        memcpy(shared_secret, &point_buf[1], keylen);
    }
...
cleanup:
    ...
    mbedtls_ecp_point_free(&z_point);
