...
void TestThumbFunc(void)
{
    printf("This is a test thumb function.\n");
    UART_WAIT_TX_EMPTY(DEBUG_PORT);
}

int32_t main(void)
{
    void (*pfnArmModeFunc)(void);
    ...
    while (1)
    {
        ...
        switch (i8ch)
        {
            ...
            case '1':
                pfnArmModeFunc = TestThumbFunc;
                pfnArmModeFunc();
                /* Call function with bit0 = 0 will cause hard fault. (Change to ARM mode hard fault) */
                pfnArmModeFunc = (void (*)(void))((uint32_t)TestThumbFunc & ~BIT0);
                pfnArmModeFunc();
                break;
