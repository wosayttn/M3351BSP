...
volatile uint8_t g_u8MonRecData[(I2C_DATA_MAX + 1) * 2] = {0};
...
void UI2C_SLV_7bit_Monitor(uint32_t u32Status)
{
    ...
    if ((u32Status & UI2C_PROTSTS_STARIF_Msk) == UI2C_PROTSTS_STARIF_Msk)
    {
        ...
        if (g_u32ProtOn == 0)
        {
            g_u8MonRecEachState[g_u8MonCountS++] = GET_Mon_START;
            g_u32ProtOn = 1;
            DbgPrintf("@@ Get STA\n");
        }
        else
        {
            g_u8MonRecEachState[g_u8MonCountS++] = GET_Mon_RESTART;
    ...
    {
        DbgPrintf("@@ Get ACK\n");
        UI2C0->PROTSTS = UI2C_PROTSTS_ACKIF_Msk;
        g_u8MonRecEachState[g_u8MonCountS++] = GET_Mon_ACK;

        if (s_Event == SLAVE_ADDRESS_ACK)
        {
            if ((UI2C0->PROTSTS & UI2C_PROTSTS_SLAREAD_Msk) == UI2C_PROTSTS_SLAREAD_Msk)
            {
                g_u8MonRecEachState[g_u8MonCountS++] = GET_Mon_SLV_R;
                rxdata = (unsigned char)(UI2C0->RXDAT);
            ...
            {
                g_u8MonRecEachState[g_u8MonCountS++] = GET_Mon_SLV_W;
                rxdata = (unsigned char)(UI2C0->RXDAT);
                g_u8MonRecData[g_u8MonCountD++] = rxdata;
            ...
            {
                if (((rxdata >> 1) != (UI2C0->DEVADDR0 & 0xFF)) && ((rxdata >> 1) != (UI2C0->DEVADDR1 & 0xFF)))
                {
        ...
        {
            g_u8MonRecEachState[g_u8MonCountS++] = GET_Mon_Data;
            rxdata = (unsigned char)(UI2C0->RXDAT);
            g_u8MonRecData[g_u8MonCountD++] = rxdata;
            DbgPrintf("@@ Data(0x%X)\n", rxdata);
        }
    ...
    else if ((u32Status & UI2C_PROTSTS_NACKIF_Msk) == UI2C_PROTSTS_NACKIF_Msk)
    {
        ...
        g_u8MonRecEachState[g_u8MonCountS++] = GET_MON_NACK;
    ...
    else if ((u32Status & UI2C_PROTSTS_STORIF_Msk) == UI2C_PROTSTS_STORIF_Msk)
    {
        ...
        g_u8MonRecEachState[g_u8MonCountS++] = GET_MON_STOP;
        DbgPrintf("@@ Get STOP\n\n");
        g_u32ProtOn = 0;
        s_Event = SLAVE_ADDRESS_ACK;
        UI2C_SET_CONTROL_REG(UI2C0, UI2C_CTL_AA);
...
void I2C_SlaveTRx_7bit_1(uint32_t u32Status)
{
                                  ...
                                  Data has been received; ACK has been returned*/
    {
        u32Tmp = (unsigned char)(I2C1->DAT);
        DbgPrintf("I2CS << RXDAT: 0x%X\n", u32Tmp);
        g_au8RxData[g_u8SlvDataLen] = u32Tmp;
        g_u8SlvDataLen++;
        ...
        if (g_u8SlvDataLen == 2)
        {
            ...
            slave_buff_addr = u32Tmp;
        }

        if (g_u8SlvDataLen == I2C_DATA_MAX - 2)
        {
            u32Tmp =  g_au8RxData[I2C_DATA_MAX - 2];
            g_u8SlvData[slave_buff_addr] = u32Tmp;
            g_u8SlvDataLen = 0;
        }
