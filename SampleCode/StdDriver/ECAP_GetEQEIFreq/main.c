...
void ECAP0_IRQHandler(void)
{
    ...
    if ((u32Status & ECAP_STATUS_CAPTF0_Msk) == ECAP_STATUS_CAPTF0_Msk)
    {
        ...
        u32IC0Hold = ECAP_GET_CNT_HOLD_VALUE(ECAP0, ECAP_IC0);
    }
...
static void ECAP0_Init(void)
{
    ...
    ECAP_SET_CNT_CLEAR_EVENT(ECAP0, ECAP_CTL1_CAP0RLDEN_Msk);
...
static void EQEI0_Init(void)
{
    EQEI_Open(EQEI0, EQEI_CTL_X4_FREE_COUNTING_MODE, 0U);
    EQEI_Start(EQEI0);
}
...
int main(void)
{
    ...
    u32Status = ECAP_GET_INT_STATUS(ECAP0);
    ECAP_CLR_CAPTURE_FLAG(ECAP0, u32Status);
    u32IC0Hold = 0;
