...
int  program_flash_page(uint32_t page_addr, uint32_t *buff, int count)
{
    ...
    uint32_t  tmp;                          /* temporary data buffer                      */

    printf("Program page 0x%x, count=%d\n", page_addr, count);

    FMC_Erase(page_addr);                   /* Erase an APROM page                        */

    for (addr = page_addr; addr < page_addr + count; addr += 8, p++)    /* loop page      */
    {
        tmp = *p;
        p++;
        FMC_Write8Bytes(addr, tmp, *p);               /* program flash                              */
    }
...
void usbh_firmware_update()
{
    ...
    /*------------------------------------------------------------------------------------*/
    uint32_t   dfba;
    dfba = FMC_DATA_FLASH_BASE;     /* get Data Flash base address                */
