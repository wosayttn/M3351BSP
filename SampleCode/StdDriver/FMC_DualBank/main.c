...
static volatile int32_t   s_u32DB_ProgState = DB_STATE_DONE;    /* dual bank background program state       */
static volatile uint32_t  s_u32DB_RemainBytes;                  /* dual bank program remaining length       */
static volatile uint32_t  s_u32DB_ProgAddr;                     /* dual bank program current flash address  */
...
void SysTick_Handler(void)
{
    ...
    u32TickCnt = s_u32TickCnt;

    if (s_u32DB_ProgState == DB_STATE_DONE)     /* Background program is in idle state      */
    {
        return;
    }

    if (s_u32DB_RemainBytes == 0)               /* Background program done?                 */
    {
        s_u32DB_ProgState = DB_STATE_DONE;      /* enter idle state                         */
        return;
    }
     ...
     */
    switch (s_u32DB_ProgState)
    {
        case DB_STATE_START:
            if (s_u32DB_ProgAddr & ~FMC_PAGE_ADDR_MASK)
            {
                printf("Warning - dual bank start address is not page aligned!\n");
                s_u32DB_ProgState = DB_STATE_FAIL;
                break;
            }

            if (s_u32DB_RemainBytes & ~FMC_PAGE_ADDR_MASK)
            {
            ...
            s_u32DB_ProgState = DB_STATE_ERASE;         /* Next state is to erase flash            */
            break;

        case DB_STATE_ERASE:
            printf("Erase 0x%x [%d]\n", s_u32DB_ProgAddr, u32TickCnt);
            FMC->ISPCMD  = FMC_ISPCMD_PAGE_ERASE;       /* ISP page erase command                   */
            FMC->ISPADDR = s_u32DB_ProgAddr;            /* page address                             */
            FMC->ISPTRG  = FMC_ISPTRG_ISPGO_Msk;        /* trigger ISP page erase and no wait       */

            s_u32DB_ProgState = DB_STATE_PROGRAM;       /* Next state is to program flash           */
            break;

        case DB_STATE_PROGRAM:
            if ((s_u32DB_ProgAddr & ~FMC_PAGE_ADDR_MASK) == 0)
                printf("Erase done [%d]\n", u32TickCnt);

            FMC->ISPCMD  = FMC_ISPCMD_PROGRAM_64;       /* ISP 64-bit program command               */
            FMC->ISPADDR = s_u32DB_ProgAddr;            /* program address                          */
            FMC->MPDAT0  = s_u32DB_ProgAddr;            /* 32-bits data to be programmed            */
            FMC->MPDAT1  = s_u32DB_ProgAddr + 4;        /* 32-bits data to be programmed            */
            FMC->ISPTRG  = FMC_ISPTRG_ISPGO_Msk;        /* trigger ISP program and no wait          */

            s_u32DB_ProgAddr += 8;                      /* advance to next word                     */
            s_u32DB_RemainBytes -= 8;

            if ((s_u32DB_ProgAddr & ~FMC_PAGE_ADDR_MASK) == 0)
            {
                /* have reached start of next page          */
                s_u32DB_ProgState = DB_STATE_ERASE;     /* next state, erase page                   */
            }

            break;

        default:
            printf("Unknown s_u32DB_ProgState state!\n");
            break;
    }
}
...
void StartTimer0(void)
{
    ...
    TIMER_SET_PRESCALE_VALUE(TIMER0, ((__HIRC / FREQ_1MHZ) << TIMER_CTL_PSC_Pos) - 1);
...
int main(void)
{
    ...
    printf("+-------------------------------------+\n");
    printf("|      FMC Dual Bank Sample Code      |\n");
    printf("+-------------------------------------+\n");
    ...
    s_u32DB_ProgState = DB_STATE_DONE;  /* Dual bank program state idle     */
    s_u32CRC = 0;
    ...
    while (s_u32DB_ProgState != DB_STATE_DONE) ;  /* Wait until the dual bank program is done */
