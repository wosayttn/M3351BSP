...
uint32_t Get_CANFD_NominalBitRate(CANFD_T *psCanfd)
{
    ...
    if (CLK_GetModuleClockSource(CANFD0_MODULE) == (CLK_CLKSEL0_CANFD0SEL_HXT >> CLK_CLKSEL0_CANFD0SEL_Pos))
        u32CanClk = CLK_GetHXTFreq();
    else if (CLK_GetModuleClockSource(CANFD0_MODULE) == (CLK_CLKSEL0_CANFD0SEL_PLL >> CLK_CLKSEL0_CANFD0SEL_Pos))
        u32CanClk = CLK_GetPLLClockFreq();
    else if (CLK_GetModuleClockSource(CANFD0_MODULE) == (CLK_CLKSEL0_CANFD0SEL_HCLK >> CLK_CLKSEL0_CANFD0SEL_Pos))
        u32CanClk = CLK_GetHCLKFreq();
    else if (CLK_GetModuleClockSource(CANFD0_MODULE) == (CLK_CLKSEL0_CANFD0SEL_HIRC >> CLK_CLKSEL0_CANFD0SEL_Pos))
        u32CanClk = __HIRC;
    else
        return 0;
