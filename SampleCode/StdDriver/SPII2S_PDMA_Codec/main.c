...
//------------------------------------------------------------------------------
__ALIGNED(32) DESC_TABLE_T g_asDescTable_TX[2], g_asDescTable_RX[2];
uint32_t PcmRxBuff[2][BUFF_LEN];
uint32_t PcmTxBuff[2][BUFF_LEN];
...
void PDMA0_IRQHandler(void)
{
    ...
    uint8_t u8TxIdx1 = u8TxIdx, u8RxIdx1 = u8RxIdx;
    ...
    else if (u32Status & 0x2)
    {
        if (PDMA_GET_TD_STS(PDMA0) & 0x2)            /* channel 1 done */
        {
            ...
            memcpy(&PcmTxBuff[u8TxIdx1 ^ 1], &PcmRxBuff[u8RxIdx1], BUFF_LEN * 4);
