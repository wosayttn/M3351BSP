...
void PDMA_Init(void)
{
    ...
    PDMA_SetTransferMode(PDMA0, PDMA_I2S_TX_CH, PDMA_SPI0_TX, 1, (uint32_t)&DMA_TXDESC[0]);
    PDMA_SetTransferMode(PDMA0, PDMA_I2S_RX_CH, PDMA_SPI0_RX, 1, (uint32_t)&DMA_RXDESC[0]);
...
void PDMA_WriteTxSGTable(void)
{
    ...
    for (u16Cnt = 0; u16Cnt < PDMA_TXBUFFER_CNT; u16Cnt++)
    {
        ...
        DMA_TXDESC[u16Cnt].dest = (uint32_t)&SPI0->TX;
        ...
        if (u16Cnt != PDMA_TXBUFFER_CNT - 1)
            ...
            DMA_TXDESC[u16Cnt].offset = (uint32_t)&DMA_TXDESC[0];
    }
}
...
void PDMA_WriteRxSGTable(void)
{
    ...
    for (u16Cnt = 0; u16Cnt < PDMA_RXBUFFER_CNT; u16Cnt++)
    {
        ...
        DMA_RXDESC[u16Cnt].src = (uint32_t)&SPI0->RX;
        ...
        if (u16Cnt != (PDMA_RXBUFFER_CNT - 1))
            ...
            DMA_RXDESC[u16Cnt].offset = (uint32_t)&DMA_RXDESC[0];
    }
}
