...
int32_t main(void)
{
    ...
    uint32_t u32ReadData0, u32ReadData1;
    int32_t i32ISPSTS = 0;
    int i8GetCh;
    ...
    printf("+-----------------------------------+\n");
    printf("|      FMC APWPROT Sample Code      |\n");
    printf("+-----------------------------------+\n");
    ...
    for (u32Addr = u32StartAddr; u32Addr < u32EndAddr; u32Addr += 8)
    {
        ...
        if (FMC_Write8Bytes(u32Addr, u32Addr, u32Addr + 4) != FMC_OK || FMC_GET_FAIL_FLAG())
        {
            printf("[Error] Program failed at addr 0x%08X !\n", u32Addr);
            goto lexit;
        }

        if (FMC_Read8Bytes(u32Addr, &u32ReadData0, &u32ReadData1) != FMC_OK || FMC_GET_FAIL_FLAG())
        {
            printf("[Error] Read failed at addr 0x%08X !\n", u32Addr);
            goto lexit;
        }
    ...
    for (i = 0; i < 32; i++)
    {
        if (FMC_IS_APWPROT((i / 32), 1 << (i & 0x1F)))
        {
            ...
            for (u32Addr = u32StartAddr; u32Addr < u32EndAddr; u32Addr += 8)
            {
                ...
                if (FMC_Write8Bytes(u32Addr, 0, 0) != FMC_OK || FMC_GET_FAIL_FLAG())
                {
                    FMC_CLR_FAIL_FLAG();
                }
                else
                {
                    printf("(%d) [Error] ISP fail flag should be set when block is protected !\n", __LINE__);
                    goto lexit;
                }
