...
{
    uint8_t u8DataLen0 = g_u8DataLen0;
    ...
    else if (u32Status == 0x50)
    {
        if (g_u8DataLen0 == 0)
        {
            g_au8RxData[u8DataLen0] = (unsigned char)I2C_GET_DATA(I2C0);
            I2C_SET_CONTROL_REG(I2C0, I2C_CTL_SI_AA);
            g_u8DataLen0++;
        }
        else
...
{
    uint8_t u8DataLen1 = g_u8DataLen1;
    ...
    {
        g_au8RxData[u8DataLen1] = (unsigned char)I2C_GET_DATA(I2C1);
        g_u8DataLen1++;

        if (g_u8DataLen1 == 2)
        {
            u32Temp = (g_au8RxData[0] << 8);
            u32Temp += g_au8RxData[1];
            g_u32SlvBufAddr = u32Temp;
        }

        if (g_u8DataLen1 == 3)
        {
            g_u8SlvData[u32SlvBufAddr] = g_au8RxData[2];
        }
    ...
    {
        I2C_SET_DATA(I2C1, g_u8SlvData[g_u32SlvBufAddr]);
        g_u32SlvBufAddr++;
        I2C_SET_CONTROL_REG(I2C1, I2C_CTL_SI_AA);
    }
...
void I2C_SlaveAlert(uint32_t u32Status)
{
                                                   ...
                                                   Data has been received; ACK has been returned*/
    {
        ...
        if (g_u8DataLen1 == 3)
        {
            u32Temp = g_au8RxData[2];
            g_u8SlvData[u32SlvBufAddr] = u32Temp;
        }
...
int32_t SYS_Init(void)
{
    ...
    GPIO_ENABLE_SCHMITT_TRIGGER(PA, (BIT2 | BIT3 | BIT4 | BIT5));
    CLK_EnableModuleClock(GPC_MODULE);
    GPIO_ENABLE_SCHMITT_TRIGGER(PC, (BIT4 | BIT5));
    CLK_EnableModuleClock(GPH_MODULE);
    GPIO_ENABLE_SCHMITT_TRIGGER(PH, (BIT8 | BIT9));
