 ...
 */
static uint32_t CalNewDutyCMR(PWM_T *pwm, uint32_t u32ChannelNum, uint32_t u32DutyCycle, uint32_t u32CycleResolution)
{
    return (u32DutyCycle * (PWM_GET_CNR(pwm, u32ChannelNum) + 1U) / u32CycleResolution);
}
...
int main(void)
{
    ...
    printf("+-----------------------------------------------------------------------------------+\n");
    printf("|                     PWM SwitchDuty Sample Code                                   |\n");
    printf("+-----------------------------------------------------------------------------------+\n");
    ...
    PWM_ConfigOutputChannel(PWM0, PWM_OUT_CH, 1800, 50);
    ...
    while (1)
    {
        ...
        u32NewCMR = CalNewDutyCMR(PWM0, PWM_OUT_CH, u32NewDutyCycle, 100);
        /* Set new comparator value to register */
        PWM_SET_CMR(PWM0, PWM_OUT_CH, u32NewCMR);
    }
