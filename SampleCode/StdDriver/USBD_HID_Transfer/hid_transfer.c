...
int32_t HID_CmdReadPages(CMD_T *pCmd)
{
    ...
    if (u32Pages)
    {
        ...
        /* Trigger HID IN */
        USBD_MemCopy((uint8_t *)(USBD_BUF_BASE + USBD_GET_EP_BUF_ADDR(EP2)), (void *)g_au8PageBuff, EP2_MAX_PKT_SIZE);
...
void HID_GetOutReport(uint8_t *pu8EpBuf, uint32_t u32Size)
{
    ...
    if ((u8Cmd == HID_CMD_WRITE) && (u32PageCnt < u32Pages))
    {
        ...
        USBD_MemCopy(&g_au8PageBuff[g_u32BytesInPageBuf], pu8EpBuf, EP3_MAX_PKT_SIZE);
...
void HID_SetInReport(void)
{
    ...
    if (u8Cmd == HID_CMD_READ)
    {
        ...
        else
        {
            ...
            USBD_MemCopy(pu8Ptr, (void *)&g_au8PageBuff[PAGE_SIZE - g_u32BytesInPageBuf], EP2_MAX_PKT_SIZE);
            USBD_SET_PAYLOAD_LEN(EP2, EP2_MAX_PKT_SIZE);
            g_u32BytesInPageBuf -= EP2_MAX_PKT_SIZE;
        }
    }
