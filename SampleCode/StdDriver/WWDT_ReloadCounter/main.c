...
void SYS_Init(void)
{
    ...
    /* Set module clock*/
#if (WWDT_PORT != 0)
    CLK_SetModuleClock(WWDT1_MODULE, CLK_CLKSEL1_WWDT1SEL_LIRC, 0);
#else
    CLK_SetModuleClock(WWDT0_MODULE, CLK_CLKSEL1_WWDT0SEL_LIRC, 0);
#endif
...
int main(void)
{
    ...
    printf("      WWDT time-out period is 1024 * (64 * WWDT_CLK)      = %u ms\n", (uint32_t)dTimeOutPeriodTime);
    printf("      WWDT compare match period is 1024 * (32 * WWDT_CLK) = %u ms\n\n", (uint32_t)dCompareMatchPeriodTime);
    ...
    printf("              +------------------------+------------------------+\n");
    printf("Timeline (ms) 0                       %u                       %u\n", (uint32_t)dCompareMatchPeriodTime, (uint32_t)dTimeOutPeriodTime);
    ...
    /*
        WWDT max time-out period is 1024*(64*WWDT_CLK)
        WWDT compare value is 32 = 1024*(32*WWDT_CLK)
    ...
    */
    ...
    while (1)
    {
        ...
        /* Delay_ms < "WWDT compare match period" */
        ...
        /* Delay_ms > "WWDT max time-out period" */
        Delay_ms((uint32_t) dTimeOutPeriodTime + 100);
#elif (RELOAD_CONDITION == 3)
        ...
        else
        {
            ...
            /* "WWDT max time-out period" > Delay_ms > "WWDT compare match period" */
            Delay_ms((uint32_t) dCompareMatchPeriodTime + 50);
        }
