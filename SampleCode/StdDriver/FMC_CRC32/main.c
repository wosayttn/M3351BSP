 ...
 *           CRC32 checksum of specified FMC flash region.
 *
...
int32_t main(void)
{
    ...
    printf("\nData flash (0x%08X ~ 0x%08X) CRC32 checksum =>  ", (uint32_t)FMC_DATA_FLASH_SIZE, (uint32_t)FMC_DATA_FLASH_END);

    /*
     *  Request FMC hardware to run CRC32 calculation on LDROM.
     */
    u32ChkSum = FMC_GetChkSum(FMC_DATA_FLASH_BASE, FMC_DATA_FLASH_SIZE);
    ...
    printf("\nLDROM (0x%08X ~ 0x%08X) CRC32 checksum =>  ", (uint32_t)FMC_LDROM_BASE, (uint32_t)FMC_LDROM_END);
