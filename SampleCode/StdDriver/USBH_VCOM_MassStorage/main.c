...
#else
    BYTE workbuf[FF_MAX_SS] __attribute__((aligned(4)));
#endif
//using FAT - 12/16 format typer
MKFS_PARM opt =
{
    .fmt     = FM_FAT | FM_SFD,
    .n_fat   = 2,
...
/*--------------------------------------------------------------------------*/
/* VCOM Init/ Show information /Callback function                           */
                ...
                case 'f' :
                    switch (*ptr++)
                    {
                        ...
                        case 'e' :  /* fe - Seek file pointer */
                            ...
                            if (res == FR_OK)
                                printf("fptr=%d(0x%x)\n", (INT)file1.fptr, (uint32_t)file1.fptr);
                        ...
                        case 'm' :  /* fm - Create file system */
                            printf("FatFs rev ID = %d\n", (int)FF_DEFINED);
                            printf("The memory card will be formatted. Are you sure? (Y/n)=");
                            get_line(ptr, sizeof(Line));

                            if (*ptr == 'Y')
                                put_rc(f_mkfs(usb_path, &opt, workbuf, sizeof(workbuf)));
...
/*** (C) 2025 Nuvoton Technology Corp. All rights reserved. ***/
