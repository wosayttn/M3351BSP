...
static uint32_t SPI_GetModuleClkSrcFreq(const SPI_T *spi)
{
    ...
    switch (u32SPIClkSrcSel)
    {
        case SPI_CLKSEL_PLL_DIV2:
            u32RetValue = (CLK_GetPLLClockFreq() >> 1);          /* Clock source is PLL */
            break;
...
uint32_t SPI_Open(SPI_T *spi, uint32_t u32MasterSlave, uint32_t u32SPIMode, uint32_t u32DataWidth, uint32_t u32BusClock)
{
    ...
    // Valid data width range: 4 ~ 32 bits, except 32-bit → DWIDTH = 0
    ...
    if (u32MasterSlave == SPI_MASTER)
    {
        ...
        spi->CTL |= ((u32DataWidthTmp << SPI_CTL_DWIDTH_Pos) |
                     (u32SPIMode) |
                     SPI_CTL_SPIEN_Msk);
...
uint32_t SPII2S_Open(SPI_T *i2s, uint32_t u32MasterSlave, uint32_t u32SampleRate, uint32_t u32WordWidth, uint32_t u32Channels, uint32_t u32DataFormat)
{
    ...
    if (u32MasterSlave == SPI_MASTER)
    {
        ...
        u32SampleRateTmp = u32BitRate / (((u32WordWidth >> SPI_I2SCTL_WDWIDTH_Pos) + 1U) * 16U);
