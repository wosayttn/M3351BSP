...
void UART_Open(UART_T *uart, uint32_t u32baudrate)
{
    ...
    switch ((uint32_t)uart)
    {
        ...
        case UART6_BASE:
        case UART6_BASE+NS_OFFSET:
            u32UartClkSrcSel = CLK_GetModuleClockSource(UART6_MODULE);
            u32UartClkDivNum = CLK_GetModuleClockDivider(UART6_MODULE);
            break;

        case UART7_BASE:
        case UART7_BASE+NS_OFFSET:
            u32UartClkSrcSel = CLK_GetModuleClockSource(UART7_MODULE);
            u32UartClkDivNum = CLK_GetModuleClockDivider(UART7_MODULE);
            break;

        case UART8_BASE:
        case UART8_BASE+NS_OFFSET:
            u32UartClkSrcSel = CLK_GetModuleClockSource(UART8_MODULE);
            u32UartClkDivNum = CLK_GetModuleClockDivider(UART8_MODULE);
            break;

        case UART9_BASE:
        case UART9_BASE+NS_OFFSET:
            u32UartClkSrcSel = CLK_GetModuleClockSource(UART9_MODULE);
            u32UartClkDivNum = CLK_GetModuleClockDivider(UART9_MODULE);
            break;
...
void UART_SelectIrDAMode(UART_T *uart, uint32_t u32Buadrate, uint32_t u32Direction)
{
    ...
    {
        uart->IRDA = (uart->IRDA & (~UART_IRDA_TXEN_Msk)) | UART_IRDA_RXINV_Msk; /* Rx signal is inverse */
    }
    else
    {
        uart->IRDA = (uart->IRDA & (~UART_IRDA_TXINV_Msk)) | UART_IRDA_TXEN_Msk; /* Tx signal is not inverse */
    }
 ...
 */
void UART_SetBaudRateFractionalDivider(UART_T *uart, uint32_t u32BRFD)
{

    /* Enable Baud Rate fractional divider function */
 ...
 */
void UART_DisableBaudRateFractionalDivider(UART_T *uart)
{
    /* Disable Baud Rate fractional divider function */
