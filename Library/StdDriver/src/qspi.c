...
static uint32_t QSPI_GetPCLKSrcFreq(const QSPI_T *qspi)
{
 ...
 */
static uint32_t QSPI_GetModuleClkSrcFrq(const QSPI_T *qspi)
{
    ...
    if (qspi == QSPI0)
    {
        u32QSPIClkSrcSel = ((CLK->CLKSEL2 & CLK_CLKSEL2_QSPI0SEL_Msk) >> CLK_CLKSEL2_QSPI0SEL_Pos);
    }
    else
    {
